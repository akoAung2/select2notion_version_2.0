...
import { appendSessionMessages, loadSessionMessages } from "../lib/redisSessionManager.js";
import { synthesizeSpeech } from "../tts/ttsAdapter.js";
import type { SupportedLanguage } from "../tts/ttsTypes.js";
...
router.post("/ai/chat", requireAuth, async (req, res) => {
  ...
  // Redis is the authoritative cross-surface short-term memory. The client still
  // renders its own history, but cannot inject a different server session.
  const sessionMessages = await loadSessionMessages(req.uid!);
  const conversation = [...sessionMessages, { role: "user", content: lastUserMsg }];
...
Today: ${today}`;
      const reply = await callAI(config, systemPrompt, conversation);
      void appendSessionMessages(req.uid!, [{ role: "user", content: lastUserMsg }, { role: "assistant", content: reply.trim() }]);
...
/* POST /ai/tts — presentation-only speech synthesis for a completed AI reply. */
router.post("/ai/tts", requireAuth, async (req, res) => {
  const { text, language, voice, speed } = req.body as { text?: string; language?: SupportedLanguage; voice?: string; speed?: number };
  if (!text?.trim() || text.length > 8_000) {
    res.status(400).json({ error: "INVALID_TEXT", message: "Text is required and must be under 8,000 characters." });
    return;
  }
  const config = await getUserAIConfig(req.uid!, req.idToken!);
  if (!config) { res.status(400).json({ error: "no_ai_credentials", message: "Configure your AI provider in Settings first." }); return; }
  try {
    const result = await synthesizeSpeech(config, { text: text.trim(), language: language ?? "unknown", voice, speed });
    res.setHeader("Content-Type", result.mimeType);
    res.setHeader("Cache-Control", "no-store");
    res.send(result.audio);
  } catch (err) {
    const code = err instanceof Error ? err.message : "TTS_ERROR";
    const status = code === "RATE_LIMITED" ? 429 : code === "NETWORK_TIMEOUT" ? 504 : 422;
    res.status(status).json({ error: code, message: code === "UNSUPPORTED_LANGUAGE" ? "Voice playback is not available for this configured provider. You can still read the response." : "Voice playback unavailable. You can still read the response." });
  }
});
